...
function Vehicle() {
  ...
  // Fixed mapping functions - bidirectional conversion
  const getDisplayTypeFromApiType = (apiType) => {
    const mapping = {
      "LARGE PREMIUM": "Large: Premium",
      "LARGE STATION WAGON": "Large: Station wagon",
      "MEDIUM LOW EMISSION": "Medium: Low emission",
    ...
    return mapping[apiType] || "Large: Premium";
  };

  const getApiTypeFromDisplayType = (displayType) => {
    const mapping = {
      "Large: Premium": "LARGE PREMIUM",
      "Large: Station wagon": "LARGE STATION WAGON",
      "Medium: Low emission": "MEDIUM LOW EMISSION",
    ...
    return mapping[displayType] || "LARGE PREMIUM";
  };
  ...
  const getCurrentRate = () => {
    const selectedApiType = getApiTypeFromDisplayType(selectedCarSize);
    if (selectedVehicle?.dailyRates) {
      return getRateForVehicleType(selectedApiType);
    }
    return vehiclePrice || 840.0;
  };
  ...
  // Initialize component with Redux data
  useEffect(() => {
    console.log("Initializing with Redux data:", {
      vehicle,
      vehicleType,
      amount,
    });

    // Set vehicle type from Redux state
    if (vehicleType) {
      const displayType = getDisplayTypeFromApiType(vehicleType);
      console.log("Converting vehicle type:", vehicleType, "->", displayType);
      setSelectedCarSize(displayType);
    }

    // Set selected vehicle from Redux state
    if (vehicle && vehicleList.length > 0) {
      const vehicleId =
        vehicle && typeof vehicle === "object" ? vehicle._id : vehicle;
      if (vehicleId) {
        ...
        if (foundVehicle) {
          ...
          // Calculate rate based on vehicle type and vehicle's daily rates
          if (vehicleType && foundVehicle.dailyRates) {
            const rate = getRateForVehicleType(vehicleType);
            console.log(
              "Setting rate for vehicle type",
              vehicleType,
              "->",
              rate
            );
            setVehiclePrice(rate);
          }
        }
      }
    }

    // Set amount from Redux state
    if (amount) {
      setVehiclePrice(amount);
    }
  }, [vehicle, vehicleType, amount, vehicleList]);
  ...
  const handleCarSizeChange = (e) => {
    ...
    console.log("Car size changed:", selectedData);
    setSelectedCarSize(selectedData.carSize);

    const selectedApiType = getApiTypeFromDisplayType(selectedData.carSize);
    let newRate = selectedVehicle?.dailyRates
      ? getRateForVehicleType(selectedApiType)
      : selectedData.price;

    setVehiclePrice(newRate);

    // Update Redux store with API format
    dispatch(
      updateVehicle({
        vehicleType: selectedApiType, // Store API format in Redux
      })
    );
    dispatch(
      updateField({
  ...
  const handleVehicleChange = (vehicleId) => {
    ...
    const selectedVehicleData = vehicleList.find((v) => v._id === vehicleId);
    if (selectedVehicleData) {
      console.log("Vehicle changed:", selectedVehicleData);
      setSelectedVehicle(selectedVehicleData);

      const selectedApiType = getApiTypeFromDisplayType(selectedCarSize);
      const rateData = selectedVehicleData.dailyRates?.find(
        (rate) => rate.vehicleType === selectedApiType
      );
      const newRate = rateData ? rateData.rate : 840.0;

      console.log(
        "Setting rate for vehicle change:",
        selectedApiType,
        "->",
        newRate
      );
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg p-6 mb-8">
      ...
      <div className="mt-4 p-4 bg-gray-50 rounded text-sm">
        ...
        <p>Redux Vehicle Type (API): {vehicleType || "None"}</p>
        <p>Display Vehicle Type: {selectedCarSize}</p>
        <p>Redux Amount: {amount || "None"}</p>
        <p>Current Rate: {getCurrentRate()}</p>
        <p>Selected Vehicle: {selectedVehicle?.name || "None"}</p>
        <p>Has Daily Rates: {selectedVehicle?.dailyRates ? "Yes" : "No"}</p>
        {selectedVehicle?.dailyRates && (
          <p>
            Available Rates:{" "}
            {selectedVehicle.dailyRates
              .map((r) => `${r.vehicleType}: ${r.rate}`)
              .join(", ")}
          </p>
        )}
      </div>
